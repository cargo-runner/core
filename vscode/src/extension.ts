...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
	...
	const filename = vscode.Uri.joinPath(context.extensionUri, 'target', 'wasm32-unknown-unknown', 'debug', 'cargo_runner.wasm');
...
context.subscriptions.push(vscode.commands.registerCommand('cargo-runner.exec', () => {
	channel.show();
	channel.appendLine('Running cargo');
	...
	channel.appendLine(`Cargo Runner Result: ${result}`);
}));
}